...
- **`generate_md_docs.py`**: Generates `HOUND_API_REFERENCE.md`, `DEVELOPER_API_REFERENCE.md`, and `llm-integration-guide.md` from LDoc-annotated source. Two LLM paths for integration guide generation:
  ...
  - **opencode CLI** (`--use-opencode`): `generate_llm_integration_guide_opencode()` — one-shot generation via `opencode run` with stdin prompt. Passes `--model` when `--opencode-model` is set (uses opencode default if omitted).
